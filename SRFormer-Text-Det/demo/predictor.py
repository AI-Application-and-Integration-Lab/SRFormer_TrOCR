...
class VisualizationDemo(object):
    def __init__(self, cfg, instance_mode=ColorMode.IMAGE, parallel=False):
        ...
        # create dir and image id
        self.tmp_dir = "./tmp_crop"
    ...
    def run_on_image(self, image, path):
        ...
        if "panoptic_seg" in predictions:
            ...
            if "instances" in predictions:
                ...
                vis_output = visualizer.draw_instance_predictions(predictions=instances)
                all_scores = predictions['instances'].scores
                
                ctrl_pnts = instances.polygons.numpy()

                for ctrl_pnt_idx in range(ctrl_pnts.shape[0]):
                    score = all_scores[ctrl_pnt_idx].cpu().numpy()
                    
                    points = ctrl_pnts[ctrl_pnt_idx].reshape(-1, 2)
                    x_min = 1000000
                    x_max = 0
                    y_min = 1000000
                    y_max = 0
                    ...
                    crop_img = PIL_image.crop((x_min, y_min, x_max, y_max))
                    image_path = os.path.join(self.tmp_dir, str(self.image_id)+".png")
                    crop_img.save(image_path)
                    self.gt_list.append({"original_image": path, "crop_image": image_path, "label":"###", "poly":points.tolist()})
                    self.image_id += 1
