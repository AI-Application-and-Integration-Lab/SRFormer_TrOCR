...
def get_tl_line_values_gt(line, LTRB=True, withTranscription=False, withConfidence=False, imWidth=0, imHeight=0):
    ...
    if LTRB:
        ...
        if withTranscription and withConfidence:
            ...
            cors = line.strip().split(',')[:-1]
            recs = str(line.split(',')[-1]).strip()
        ...
        points = []
        for ipt in pts:
            points.append(float(ipt[0]))
            points.append(float(ipt[1]))
...
def get_tl_line_values(line,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0):
    ...
    if LTRB:
        ...
        arrStr = line.split(',####')
        cors = arrStr[0].split(',')
        points = list(map(float, cors))
        # numPoints = len(arrStr) - 1
        # for i in range(0, len(arrStr) - 1):
        #     points.append(float(arrStr[i]))

        if withTranscription:
            transcription = arrStr[-1]
...
def main_evaluation(p,det_file, gt_file, default_evaluation_params_fn, validate_data_fn, evaluate_method_fn, show_result=True,per_sample=True):
    ...
    if show_result:
        ...
        sys.stdout.write('\n')
        sys.stdout.write(json.dumps(resDict['det_method']))
        sys.stdout.write('\n')
