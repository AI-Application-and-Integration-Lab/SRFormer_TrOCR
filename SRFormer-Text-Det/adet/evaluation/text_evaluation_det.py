...
class TextDetEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        if "rotate" in dataset_name:
            ...
            self._text_eval_gt_path = "datasets/evaluation/gt_mlt_valid.zip"
        elif "chinese" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_chinese.zip"
        elif "latin" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_latin.zip"
        elif "japanese" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_japanese.zip"
        elif "korean" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_korean.zip"
        elif "bangla" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_bangla.zip"
        elif "arabic" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_arabic.zip"
        elif "except" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/except_latin.zip"
        elif "d503" in dataset_name:
            self._text_eval_gt_path = "../datasets/d503_SRFormer_format/test/txt_labels.zip"
    ...
    def to_eval_format(self, file_path, temp_dir="temp_det_results"):
        ...
        with open(file_path, 'r') as f:
            ...
            with open('temp_all_det_cors.txt', 'w') as f2:
                ...
                for ix in range(len(data)):
                    # print(ix,data[ix]['image_name'])
                    # exit()
                    poly_area = compute_area(data[ix]['polys'])
                    if data[ix]['score'] > 0.1:
                        outstr = '{}: '.format(data[ix]['image_name'])  # 'image_id'
        ...
        for line in fres:
            ...
            s = line.split(': ')
            filename = '{:06d}.txt'.format(int(s[0]))
