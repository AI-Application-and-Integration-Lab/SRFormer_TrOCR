...
img_path = './datasets/d503/test/images'
img_folder = os.listdir(img_path)

annotation_path = "./datasets/d503/test/labels"
annotation_folder = os.listdir(annotation_path)

tgt_folder_name = 'd503_SRFormer_format'

img_tgt_path = f"./datasets/{tgt_folder_name}/test"
label_tgt_path = f"./datasets/{tgt_folder_name}"
txt_path = f'./datasets/{tgt_folder_name}/txt_val'
...
def cal_bbox(points):
    ...
    return [round(float(left_top_point[0]),2), round(float(left_top_point[1]),2), float(h), float(w)], int(w*h), [left_top_point, left_bottom_point, right_bottom_point, right_top_point] # 
...
for id, image_name in enumerate(tqdm(img_folder)):
    ...
    for idx, label in enumerate(current_json["shapes"]):
        ...
        if label['group_id'] == 0 or label['group_id']==2:
            ...
            text_label.append(','.join(map(str, map(int,[points[0][0], points[0][1], points[1][0], points[1][1], points[2][0], points[2][1], points[3][0], points[3][1]]))))
    ...
    with open(os.path.join(txt_path, image_name[:-4]+'.txt'), 'a') as the_file:
      for e in text_label:
         the_file.write(e+',####\n')
    # exit()
...
all_dict['categories'] = [dict(id=1, name='text')] # dict(id=0, name='Ch-Str'), dict(id=2, name='En-Str')]
all_dict['annotations'] = annotations

with open( os.path.join(label_tgt_path, "test_poly_pos.json"), "w") as outfile:
